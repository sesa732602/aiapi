...
import { ApiPermission, PermissionType } from '../models/ApiPermission';
      ...
      if (api.teamId) {
        ...
        if (!teamMember || (teamMember.role !== 'owner' && teamMember.role !== 'admin')) {
          ...
          const apiPermission = await ApiPermission.findOne({ 
            where: { apiId: apiIdNum, userId, permissionType: PermissionType.ADMIN } 
          });
          if (!apiPermission) {
    ...
    if (req.user.role !== 'super_admin' && api.ownerId !== userId) {
      ...
      if (api.teamId) {
        ...
        if (!teamMember || (teamMember.role !== 'owner' && teamMember.role !== 'admin')) {
          // 检查用户是否有API管理权限
          const apiPermission = await ApiPermission.findOne({ 
            where: { apiId: plan.apiId, userId, permissionType: PermissionType.ADMIN } 
          });
          if (!apiPermission) {
