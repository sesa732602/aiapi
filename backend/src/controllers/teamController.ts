...
export const createTeam = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // 检查用户是否已经拥有团队
    const existingOwnership = await TeamMember.findOne({ 
      where: { 
        userId,
        role: 'owner'
      }
    ...
    if (existingOwnership) {
      res.status(400).json({ message: '您已经拥有一个团队，每个用户只能创建一个团队' });
      return;
    }
    ...
    if (existingMembership) {
      res.status(400).json({ message: '您已经是其他团队的成员，请先退出当前团队' });
      return;
    }
...
export const addTeamMember = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // 检查用户是否已是团队成员
    const existingMember = await TeamMember.findOne({ where: { userId: memberUserId } });
    if (existingMember) {
